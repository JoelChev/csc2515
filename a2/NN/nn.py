...
def Train(model, forward, backward, update, eps, momentum, num_epochs,
          batch_size, cnn=True, hidden_layer=False, filter_number=False):
    """Trains a simple MLP.
    ...
    for epoch in range(num_epochs):
        ...
        valid_ce, valid_acc = Evaluate(
            inputs_valid, target_valid, model, forward, 'train', batch_size=batch_size)
        print(('Epoch {:3d} '
               'Validation CE {:.5f} '
               'Validation Acc {:.5f}\n').format(
        ...
        if(cnn==False):
            ...
            nf0=model['nf0']
            nf1=model['nf1']
        DisplayPlot(train_ce_list, valid_ce_list, 'Cross Entropy', nh0=nh0, nh1=nh1, nf0=nf0, nf1=nf1, number=0, eps=eps, momentum=momentum, batchsize=batch_size, cnn=cnn, hidden_layer=hidden_layer, filter_number=filter_number)
        DisplayPlot(train_acc_list, valid_acc_list, 'Accuracy', nh0=nh0, nh1=nh1, nf0=nf0, nf1=nf1, number=1, eps=eps, momentum=momentum, batchsize=batch_size, cnn=cnn, hidden_layer=hidden_layer, filter_number=filter_number)

    print()
    train_ce, train_acc = Evaluate(
        inputs_train, target_train, model, forward, 'train', batch_size=batch_size)
    valid_ce, valid_acc = Evaluate(
        inputs_valid, target_valid, model, forward, 'validation', batch_size=batch_size)
    test_ce, test_acc = Evaluate(
        inputs_test, target_test, model, forward, 'test', batch_size=batch_size)
...
def Evaluate(inputs, target, model, forward, datatype, batch_size=-1):
    ...
    for step in range(num_steps):
        ...
        #q35
        max_probability_threshold = 2 / 7.0
        max_pred = np.max(prediction, axis=1)
        max_pred_index = np.argmax(prediction, axis=1)
        for i in range(prediction.shape[0]):
            if (max_pred[i] < max_probability_threshold):
                print(datatype)
                print('Uncertainty:')
                print(i)
                print('Top Class:')
                print(max_pred_index[i])
                print('Top Probability:')
                print(max_pred[i])
                print('All Probabilities:')
                print(prediction[i])
                print ('Image Index:')
                print (step*batch_size+i)
...
def main():
    ...
    # Train model.
    stats = Train(model, NNForward, NNBackward, NNUpdate, eps, momentum, num_epochs, batch_size, cnn=False)

    #q34
    first_layer_weights = model['W1']
    first_layer_weights = first_layer_weights.reshape(48,48,16)
    for i in range(num_hiddens[0]):
        plt.matshow(first_layer_weights[:,:,i], cmap=plt.cm.gray)
        plt.title('First Layer Weights for NN, Group' + str(i))
        plt.savefig('./plots/filter_weights/First_Layer_Weights' + str(i) + '.png')
...
def q32():
    ...
    num_inputs = 2304
    num_outputs = 7
    epsilon_array = [0.001, 0.05, 0.1, 0.5, 1.0]
    ...
    for eps_i in epsilon_array:
        ...
        stats = Train(model, NNForward, NNBackward, NNUpdate, eps_i,
                      momentum, num_epochs, batch_size, cnn=False)
    for momentum_i in momentum_array:
        model = InitNN(num_inputs, num_hiddens, num_outputs)
        stats = Train(model, NNForward, NNBackward, NNUpdate, eps,
                      momentum_i, num_epochs, batch_size, cnn=False)
    for bs_i in batch_size_array:
        model = InitNN(num_inputs, num_hiddens, num_outputs)
        stats = Train(model, NNForward, NNBackward, NNUpdate, eps,
                      momentum, num_epochs, bs_i, cnn=False)
def q33():
    eps = 0.01
    momentum = 0.9
    num_epochs = 1000
    ...
    num_inputs = 2304
    num_outputs = 7
    num_hidden_array= [[2,4], [24,48] ,[50,100]]
    for nh_i in num_hidden_array:
        model = InitNN(num_inputs, nh_i, num_outputs)
        stats = Train(model, NNForward, NNBackward, NNUpdate, eps,
                      momentum, num_epochs, batch_size, cnn=False, hidden_layer=True)
